	...
	function arReportProcess()
	 {
	  ...
	  $this->db->from('kra');
 $this->db->join('users','kra.users_id = users.users_id AND users.status="Active"')->join('kra_points','kra.kra_id = kra_points.kra_id AND'." YEAR(final_application_date) ="."'".$this->input->post('report_year')."'",'TYPE',false)->join('kra_final_review','kra.kra_id = kra_final_review.kra_id'); 
