...
class ItReport extends MY_Controller
{
    ...
    public function it_report_v()
    {
        ...
        else
        {
            ...
            $select = "SELECT ist.ticket_number,ist.issue,ist.details_of_call,ist.call_logged_via,ist.logged_by_name,ist.system_number_tag,ist.call_assigned_time,ist.remarks,ist.comment,ist.issue_type,ist.status,ist.dateTime, ist.system_number_tag ,u.users_first_name AS fname,u.users_last_name AS lname FROM `it_support_ticket` AS ist LEFT JOIN users AS u ON ist.users_id = u.users_id WHERE ist.`dateTime` >= '$f_date' AND ist.`dateTime` <= '$t_date'";
            ...
            $data_array[] = array( 'Sr No','Ticket Number','Full Name', 'System Number Tag', 'Issue', 'Details of call', 'Call logged via', 'Issue Type', 'Status', 'Date');
            if( $query->num_rows() > 0 )
            {
                $i=1;
                foreach ($query->result() as $row)
                {
                    ...
                    $data_array[] = array( $i, $row->ticket_number, $full_name, $row->system_number_tag, $row->issue, $row->details_of_call, $row->call_logged_via, $row->issue_type, $row->status, $report_date);
                $i++;
                }
            }
            else
            ...
            //$date = date('d-m-y');
            $xls->generateXML("TicketReport_".$from_date."_".$to_date);
